...
public class MultiRoomEnvironmentMeshAssemblerTests
{
    static MultiRoomEnvironment GeneratePlan(int seed, int w = 4, int h = 4, int rects = 0,
    ...
    public void Assemble_ReturnsOneMeshPerCell()
    {
        ...
        var meshes = new MultiRoomEnvironmentMeshAssembler().Assemble(plan, 0.2f, 3f);
        Assert.Equal(plan.Rooms.Count, meshes.Count);
    }
    ...
    public void Assemble_PreservesCellOrder()
    {
        ...
        for (int i = 0; i < plan.Rooms.Count; i++)
            Assert.Same(plan.Rooms[i], meshes[i].Room);
    }
    ...
    public void Assemble_EveryCellHasNonEmptyFloorAndCeiling()
    {
        ...
        foreach (var m in meshes)
        {
            Assert.True(m.Mesh.TriangleCount(SubmeshIndex.Floor) > 0,
                $"Room #{m.Room.Id} has no floor.");
            Assert.True(m.Mesh.TriangleCount(SubmeshIndex.Ceiling) > 0,
                $"Room #{m.Room.Id} has no ceiling.");
        }
    }
    ...
    {
        // Single 1×1 grid (one room). The room has 4 outer adjacencies, all closed.
    ...
    {
        // 1×2 grid → 2 square rooms (id 0 at (0,0), id 1 at (0,1) — or swapped depending
        // on iteration order; we check by counting walls).
        var plan = GeneratePlan(0, 1, 2, assign: false);   // no spanning tree → all closed
        var meshes = new MultiRoomEnvironmentMeshAssembler().Assemble(plan, 0.2f, 3f).ToList();

        Assert.Equal(2, meshes.Count);
        var byId = meshes.ToDictionary(m => m.Room.Id, m => m);

        // Room 0 has: 3 outer walls (south, west, east) + 1 shared wall to room 1 = 4 walls.
        // Room 1 has: 3 outer walls (north, west, east), shared wall is owned by room 0 = 3 walls.
    ...
    public void TwoCells_OpenAdjacency_ProducesNoSharedWallGeometry()
    {
        ...
        // Each room now only has its 3 outer walls (no shared wall material at all).
    ...
    public void TwoCells_DoorwayAdjacency_OwnerHasTunneledWall()
    {
        ...
        int ownerId = sharedAdj.RoomA.Id < sharedAdj.RoomB!.Id ? sharedAdj.RoomA.Id : sharedAdj.RoomB.Id;
        int otherId = ownerId == sharedAdj.RoomA.Id ? sharedAdj.RoomB.Id : sharedAdj.RoomA.Id;
    ...
    public void Assembler_DeterministicForSamePlan()
    {
        ...
        var a = new MultiRoomEnvironmentMeshAssembler().Assemble(plan, 0.2f, 3f);
        var b = new MultiRoomEnvironmentMeshAssembler().Assemble(plan, 0.2f, 3f);
    ...
    public void Assembler_RejectsInvalidArguments()
    {
        ...
        var asm = new MultiRoomEnvironmentMeshAssembler();
    ...
    public void Assembler_FullPipeline_SpanningTreeProducesValidMeshes()
    {
        ...
        // room ends up with non-zero mesh content (floor, ceiling, and at least one wall).
        ...
        foreach (var m in meshes)
        {
            Assert.True(m.Mesh.TriangleCount(SubmeshIndex.Floor) > 0);
            Assert.True(m.Mesh.TriangleCount(SubmeshIndex.Ceiling) > 0);
            // Most rooms will have walls; corner-only rooms with all-open might not, but
            // with the spanning-tree assigner some passages stay closed.
        }

        // Total wall triangles should match the spanning-tree expectation:
        // (rooms - 1) doorways + (outer adjacencies - 1) outer-closed walls + 1 entrance doorway +
        // (internal adjacencies - (rooms-1)) closed walls. Each closed wall = 12 tris,
