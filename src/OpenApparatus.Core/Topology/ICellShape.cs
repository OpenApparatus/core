...
/// <summary>
/// The 2D footprint of a cell in cell-local coordinates (XZ plane).
...
{
    /// <summary>
    /// The shape's outline as a closed polyline in cell-local coordinates,
    /// CCW from above (the cell interior is on the left of each edge).
    /// Ordering is shape-specific but deterministic.
    /// </summary>
    IReadOnlyList<EdgeSegment> GetOutline();

    /// <summary>Axis-aligned bounding box of the outline in cell-local coordinates.</summary>
